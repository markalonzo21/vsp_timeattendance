...
class Report extends CI_Controller
{
	...
	public function generateReport()
	{
		...
		for($i = 0; $i < count($data); $i++)
		{
			...
			$spreadsheet->getActiveSheet()->setCellValueByColumnAndRow(8, $col, date('m/d/Y', strtotime($data[$i]['date_recognized']) ));
			$spreadsheet->getActiveSheet()->setCellValueByColumnAndRow(6, $col, date('h:i:s A', strtotime($data[$i]['date_recognized']) ));
        ...
        for ($i=0; $i < count($column_array); $i++) {
        	$spreadsheet->getActiveSheet()->setCellValueByColumnAndRow(19, $dailies_col, date('h:i:s A', strtotime($data["time_ins"][$i][0]['date_recognized'])) );
        	$spreadsheet->getActiveSheet()->setCellValueByColumnAndRow(22, $dailies_col, date('h:i:s A', strtotime($data["time_outs"][$i][0]['date_recognized'])) );
        	$spreadsheet->getActiveSheet()->setCellValueByColumnAndRow(26, $dailies_col, (float)(date('h:i:s A', strtotime($data["time_outs"][$i][0]['date_recognized'])) - date('h:i:s A', strtotime($data["time_ins"][$i][0]['date_recognized']))) );
        	$dailies_col++;
        }
